// MODIFIED FOR THIS PROJECT — see the notes marked [Kagemura] below.
// This template ships against the legacy Input Manager, but the project runs on the Input System
// package alone (Player Settings > Active Input Handling = "Input System Package (New)"). Under
// that setting every UnityEngine.Input call throws InvalidOperationException, so the menu threw
// once per frame and no key did anything. The reads are ported to Keyboard.current here.
//
// Re-importing or updating this Asset Store package will overwrite this file and reintroduce all
// of it.
using UnityEngine;
...
public class MenuController : MonoBehaviour {
	...
	void Update () {
        ...
        else
        {
            ...
            //[Kagemura] Ported to the Input System. Null-checked rather than early-returned: the
            //isAnimating check below this block still has to run on a machine with no keyboard.
            var keyboard = Keyboard.current;
            if (useKeys && keyboard != null)
            {
    ...
    public void newGame()
    {
        //[Kagemura] Was SceneManager.LoadScene(1) — a raw build index, which silently loads
        //whatever happens to sit at slot 1 and breaks the moment the Build Settings order changes.
        //SceneRoutes loads by name, says so plainly when the scene is not in Build Settings, and
        //resets timeScale on the way out so the next scene never starts frozen.
        SceneRoutes.Load(SceneRoutes.DefaultFirstLevel);
    }
    ...
    {
        //[Kagemura] Application.Quit is a no-op in the editor, so the Exit option looked broken
        //while testing. SceneRoutes.Quit stops play mode instead and says why.
        SceneRoutes.Quit();
    }
